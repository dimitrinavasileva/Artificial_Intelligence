...
public class Classifier {
    ...
    private static final String dataPath = new File("house-votes-84.data").getAbsolutePath();

    public Classifier() {
        readData();
        size = rows.size();
        testSize = size / FOLD_K;
        table = new ProbabilityTable[size];
    ...
    private void teach(int testStart, int testEnd) {
        for (int i = 0; i < 17; i++) {
            ...
            //Filling the probability tables
            ...
            table[i] = new ProbabilityTable(yes, no, other);
        }
    }
